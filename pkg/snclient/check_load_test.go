...
func TestCheckLoad(t *testing.T) {
	...
	assert.Regexpf(t,
		`^CRITICAL - total load average: [\d\.]+, [\d\.]+, [\d\.]+ on \d+ cores \|'load1'=[\d\.]+;0;0;0 'load5'=[\d\.]+;0;0;0 'load15'=[\d\.]+;0;0;0$`,
	...
	assert.Regexpf(t,
		`total load average: [\d\.]+, [\d\.]+, [\d\.]+ on \d+ cores \|'load1'=[\d\.]+;0;999;0 'load5'=[\d\.]+;0;998;0 'load15'=[\d\.]+;0;997;0$`,
		string(res.BuildPluginOutput()),
		"output matches",
	)
