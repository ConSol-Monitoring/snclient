...
func (checkInterrupt *CheckInterrupt) Check(ctx context.Context, snc *Agent, check *CheckData, _ []Argument) (*CheckResult, error) {
	...
	needInterruptController := check.HasThreshold("interrupt_controller")
	needInterruptPciBdf := check.HasThreshold("interrupt_pci_bdf")
	needInterruptPinNameVector := check.HasThreshold("interrupt_pin_name_vector")
	needDeviceAndDriverName := check.HasThreshold("interrupt_device_and_driver_name")
	for _, data := range check.listData {
		...
		if needInterruptController {
			check.result.Metrics = append(check.result.Metrics,
				&CheckMetric{
					ThresholdName: "interrupt_controller",
					Name:          fmt.Sprintf("id: %s ; name: %s ; cpu: %s ; interrupt_count: %s", data["interrupt_number"], data["interrupt_name"], data["cpu"], data["interrupt_count"]),
					Value:         data["interrupt_controller"],
					Unit:          "",
					Warning:       check.warnThreshold,
					Critical:      check.critThreshold,
				},
		...
		if needInterruptPciBdf {
			check.result.Metrics = append(check.result.Metrics,
				&CheckMetric{
					ThresholdName: "interrupt_pci_bdf",
					Name:          fmt.Sprintf("id: %s ; name: %s ; cpu: %s ; interrupt_count: %s", data["interrupt_number"], data["interrupt_name"], data["cpu"], data["interrupt_count"]),
					Value:         data["interrupt_pci_bdf"],
					Unit:          "",
					Warning:       check.warnThreshold,
					Critical:      check.critThreshold,
				},
		...
		if needInterruptPinNameVector {
			check.result.Metrics = append(check.result.Metrics,
				&CheckMetric{
					ThresholdName: "interrupt_pin_name_vector",
					Name:          fmt.Sprintf("id: %s ; name: %s ; cpu: %s ; interrupt_count: %s", data["interrupt_number"], data["interrupt_name"], data["cpu"], data["interrupt_count"]),
					Value:         data["interrupt_pin_name_vector"],
					Unit:          "",
					Warning:       check.warnThreshold,
					Critical:      check.critThreshold,
				},
