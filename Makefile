...
# test with filter
testf: vendor
	CGO_ENABLED=$(CGO_ENABLED) $(GO) test -count=1 -v $(TEST_FLAGS) ./pkg/* ./pkg/*/commands -run "$(filter-out $@,$(MAKECMDGOALS))" 2>&1 | grep -v "no test files" | grep -v "no tests to run" | grep -v "^PASS"
...
golangci: tools
	...
	@echo "  - GOOS=linux"; \
	GOOS=linux CGO_ENABLED=0 golangci-lint run $(GOLANG_CI_OPTIONS) pkg/... cmd/... t/...
	@echo "  - GOOS=darwin"; \
	GOOS=darwin CGO_ENABLED=$(shell if [ "$(GOOS)" != "darwin" ]; then echo "0"; else echo $(CGO_ENABLED); fi ) golangci-lint run $(GOLANG_CI_OPTIONS) pkg/... cmd/... t/...
	@echo "  - GOOS=freebsd"; \
	GOOS=freebsd CGO_ENABLED=0 golangci-lint run $(GOLANG_CI_OPTIONS) pkg/... cmd/... t/...
	@echo "  - GOOS=windows"; \
	GOOS=windows CGO_ENABLED=0 golangci-lint run $(GOLANG_CI_OPTIONS) pkg/... cmd/... t/...
...
dist:
	...
	if [ "$(GOOS)" = "windows" ]; then cp -p ./snclient ./dist/snclient.exe; else cp -p ./snclient ./dist/snclient; fi
