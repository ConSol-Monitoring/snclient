...
WINDOWS_EXPORTER_VERSION=0.30.8
...
golangci: tools
	...
	@echo "  - GOOS=darwin"; \
	GOOS=darwin CGO_ENABLED=$(shell if [ "$(GOOS)" != "darwin" ]; then echo "0"; else echo $(CGO_ENABLED); fi ) golangci-lint run --timeout=5m pkg/... cmd/... t/...
	@echo "  - GOOS=freebsd"; \
